    ...
    if (error || !user) {
        return (
            <div className="max-w-2xl mx-auto">
                ...
                <div className="mt-4">
                    <Link to="/users" className="text-blue-600 hover:text-blue-800 font-medium font-sans">
        ...
        <div className="max-w-3xl mx-auto font-sans">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                        Visualisation des informations complètes du compte.
                    </p>
                </div>
                <div className="flex space-x-3">
                    ...
                    <Link
                        ...
                        className="inline-flex items-center justify-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 transition-all font-sans"
                    >
                        Modifier
                    </Link>
                </div>
            ...
            <div className="bg-white shadow-sm sm:rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="px-6 py-6 sm:p-8">
                    <h4 className="text-lg font-semibold text-slate-800 mb-4 border-b border-slate-100 pb-2">Détails d'accès</h4>
                    <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        <div className="sm:col-span-1">
                            <dt className="text-sm font-medium text-slate-500">ID Système</dt>
                            <dd className="mt-1 text-sm text-slate-900 bg-slate-50 p-2 rounded border border-slate-100 font-mono">#{user.id}</dd>
                        </div>
                        <div className="sm:col-span-1">
                            <dt className="text-sm font-medium text-slate-500">Succursale (Agence ID)</dt>
                            <dd className="mt-1 text-sm font-semibold text-slate-900 bg-slate-50 p-2 rounded border border-slate-100">{user.agence_id}</dd>
                        </div>
                        <div className="sm:col-span-1">
                            <dt className="text-sm font-medium text-slate-500">Rôle d'administration</dt>
                            <dd className="mt-1 text-sm text-slate-900 bg-slate-50 p-2 rounded border border-slate-100 flex items-center">
                                ...
                                {user.role === 'admin' ? 'Administrateur complet' : 'Employé standard'}
                            </dd>
                        </div>
                        <div className="sm:col-span-1">
                            <dt className="text-sm font-medium text-slate-500">Statut de connexion</dt>
                            <dd className="mt-1 text-sm text-slate-900 bg-slate-50 p-2 rounded border border-slate-100 flex items-center">
                                {user.actif ? 'Autorisé à se connecter' : 'Connexion bloquée'}
                            </dd>
                        </div>
                    </dl>
                </div>
